...
We now introduce the Measure thoery basics required to define the probability measure for our percolation model.
\begin{definition}[$\sigma$-algebra]
	For some set $X$, we call $\mathcal{A} \subseteq \mathcal{P}(X)$ a  $\sigma$-algebra if:
	\begin{enumerate}
	...
	\end{enumerate}
	We call the elements of $\mathcal{A}$ measurable sets.
\end{definition}
...
\begin{proof}
	...
	\end{lemma}
	\begin{proof}
		{Thinking visually since there is a ring of closed edges in the dual, the open edges from the origin in the original graph cannot extend beyond this ring. A more formal pure graph theory proof can be made but is ommitted here.  }
	\end{proof}
