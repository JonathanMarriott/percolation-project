...
\begin{corollary}
	...
	Where $0\stackrel{S}{\longleftrightarrow}y$ occurs if $0$ is connected to $y$ by open edges in $S$.
	And where $\partial S$ denotes the vertex boundary of S, ie $\forall s \in \partial S$, the neighbourhood of s contains some $x \in \ints^d \setminus S$
\end{corollary}
\begin{proof}
	{\color{red}{todo}}
...
\section{Simulation}
Discuss various ways of exploring percolation via simulations.
\subsection{The "water level" of open crossings in finite boxes}
Discuss how uniform R.V.s can be assigned to edges to simulate percolation. 
