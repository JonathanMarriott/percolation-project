...
\includepdf[pages=-]{project_sources_acknowledge.pdf}
...
Broadly percolation theory can be split into 2 main categories: bond percolation and site percolation. Given some initial graph, in site percolation we assign the vertices of the graph to be open or closed. Whereas in bond percolation we assign edges to be open or closed, see Figure~\ref{fig:simBox}. Due to historic reasons the terminology used in percolation theory differs from that used in modern graph theory, subsequently graph edges and vertices are often referred to as bonds and sites respectively. Throughout this report we will be largely focusing on bond percolation, although it is interesting to note that all bond percolation problems can be reformulated as site percolation problems but not always the other way around \cite{grimmett1999percolation}.
...
However we cannot just take $\mathcal{F} = \mathcal{P}(\Omega)$, the sigma algebra of all subsets of the configuration space on $\ints^d$. This is because we would like for the \sigalg to be invariant under the symmetries of $\ints^d$, but it is not possible to have this property and also ensure every set of configurations can be assigned a probability when we take $\mathcal{F} = \mathcal{P}(\Omega)$.
Therefore, we use the \sigalg generated by the cylinder sets of $\Omega$ to avoid this issue. We base our definition on the one given by \cite{bollo2006}.
...
Where again $q = 1-p$, and we use the subscript $p$ in $\prob$ to emphasise that $p$ is the parameter in our model, such that later if we fix $p$ to a specific value it will be clear.
...
\begin{definition}[Increasing Event]
	...
	We notice that the event $\{|C| = \infty\}$ is clearly an increasing event, since adding open edges to a configuration with an infinite connected open cluster containing the origin will not remove the infinite cluster.
...
\begin{proof}
	...
	\begin{align*}
	 ...
	 \end{align*}
	 Since $p < 1/3$ clearly the limit on the final line is equal to zero. Thus, we have $\theta(p)=\lim_{n\rightarrow \infty}\prob(X_n) = 0$.
\end{proof}
...
$$p_c(d) \geq  p_c(d+1) $$
Since, if we couple the configurations of each lattice as mentioned by embedding $\ints^d$ into $\ints^{d+1}$ at the origin. Imagine as the value of $p$ increases from 0 to 1, the origin lies in an infinite open cluster of $\ints^{d+1}$ at worst at the same value of $p$ for which the origin lies in an infinite open cluster of $\ints^d$.
...
\begin{theorem}[Harris-Kesten theorem]
	The critical value on $\ints^2$ is $p_c(2) =  \frac{1}{2}$.
\end{theorem}
The aim of this section is to construct a proof of this theorem. We largely follow the proofs and methods of \cite{bollobas2006short}, which in turn is largely based on methods by Russo, Seymour and Welsh.
...
\begin{proof}
	...
	Note that we can just show the inequality holds when we assume $f(0) = g(0) = 0$ since any additional constants in $f$ and $g$ would not affect the inequality. 
	...
	Then letting $ \mathbb{P}_{\omega_{k+1}}$ be the law of $\omega_{k+1}$ and similarly $\mathbb{E}_{\omega_{k+1}}$.
	We see that 
	\begin{align*}
		\expp(fg|\omega_1,...,\omega_k) &= p f(\omega_1,...,\omega_k,1)g(\omega_1,...,\omega_k,1)\\
	&+ qf(\omega_1,...,\omega_k,0)g(\omega_1,...,\omega_k,0)\\
...
\begin{corollary}
	...
	$$\prob(A|B) \geq \prob(A)$$
\end{corollary}
Intuitively this makes sense, the probability A occurs increases when we know more edges are open, and knowing B has occurred tells us about open edges since it is also increasing.
...
\begin{proof}
	...
	Finally we get that the event there is a vertical crossing of $S$, denoted $V(S)$, is the disjoint union of the events ${LV(S) = P_1}$ for all $P_1$ in $S$. Subsequently, we get that:
...
We do not give the proof since it is too long to include within the scope of this project, but the reader is recommended to view \cite{friedgut1996every} for details. This theorem tells us all symmetric increasing events have a `sharp threshold', meaning a small increase of order $1/log|X|$ to the parameter $p$ is enough to take the probability of the event from a value close 0 to nearly 1, see Figure~\ref*{fig:sharpThreshold}.
...
\subsection{Percolation simulation design}
...
My plan to investigate the percolation threshold for the planar square lattice was to simulate many boxes each of increasing width to see how the variation in the crossing threshold decreases with box width. Using the previously described algorithms to find the threshold for each box.
...
\begin{landscape}
	\begin{figure}
		\centering
		\makebox[3\textwidth][l]{\includegraphics[scale=0.48]{Simulation_with_stddev_lines.png}}
