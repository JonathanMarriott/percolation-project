...
We now move onto proving Kesten's theorem, specifically that the percolation threshold is less than or equal to a half. Again we follow the methods detailed in \cite{bollobas2006short}. First we introduce a very powerful result due to Friedgut and
Kalai \cite{friedgut1996every} which we give without proof.

\begin{theorem}[Sharp threshold theorem]
	
\end{theorem}

We do not give the proof since it is too long to induce within the scope of this project, but the reader is recommended to view the corresponding reference for details. 
