...
\begin{definition}[d-dimensional cubic lattice]{
	...
	We will often refer to this lattice by the vertex set $\ints^d$ without specifing the edge set.
}
\end{definition}
...
We can think of the open and closed edges defining a subgraph of $\ints^d$ where only edges set to open are retained.
...
We show the existance of the critical value in this case by bounding it from above and below.
\begin{theorem}
	If $p < 1/3$, $\theta(p) = 0$.
\end{theorem}
\begin{proof}
...
\begin{proof}
	...
	This sum is finite when $q<1/3$, which is when $p>2/3$. We can make the sum arbitrarily small when $p \rightarrow 1$, when the sum is smaller than 1 this imples $\theta(p) > 0$
\end{proof}
Hence $p_c \in (1/3,1)$
