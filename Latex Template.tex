...
\begin{definition}[d-dimensional cubic lattice]{
	We construct the lattice with vertices $\ints^d$ and edges between vertices when the distance between them is one.
	$$E(\ints^d) = \{u,v \in V(\ints^d): \delta(u,v) = 1)\}$$
	We will often refer to this lattice as simply $\ints^d$ without specifing the edge set.
}
\end{definition}
